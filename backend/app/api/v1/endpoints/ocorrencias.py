...
async def criar_ocorrencia(
    ...
    current_user = Depends(get_current_user), # pega o usuário autenticado do token
):
    # Determina o ID do autor (seja via token ou via form)
    autor_id = current_user.id if current_user else usuario_id

    if midia.content_type not in ALLOWED_MIME_TYPES:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Tipo de arquivo não permitido: {midia.content_type}."
        )
